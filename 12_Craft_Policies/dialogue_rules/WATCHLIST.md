# Watchlist - Patrones sospechados

Patrones en observacion. No son reglas todavia. Si se confirman en dos escenas o el autor los senala como defecto recurrente, promover a `NN-nombre.md`.

## Separador de clase como respuesta automatica

Riesgo: que cada conversacion Cole/Chiara temprana cierre con "barrio vs elite" de forma demasiado explicita. La diferencia de mundos debe verse en gesto, costo, ruta, ropa, olor y permiso, no siempre nombrarse.

## Dario como frase perfecta de poder

Riesgo: que Dario cierre toda intervencion con una maxima impecable sobre la ciudad. Debe medir y asignar valor; si cada linea suena a lema, se vuelve villano literario en vez de poder administrativo.

## Italiano como decoracion

Riesgo: usar italiano para recordar que Chiara es italiana. Regla vigente de voz: aparece cuando esta comoda o furiosa, no como perfume constante.
