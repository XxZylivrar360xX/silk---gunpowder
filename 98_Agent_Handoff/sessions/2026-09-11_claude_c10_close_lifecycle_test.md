...
**Hueco de política detectado:** `CHAPTER_LIFECYCLE.md` nunca declara la equivalencia de forma explícita — sólo usa la frase como ilustración de la convención antigua, sin una regla que diga "todo capítulo heredado con `Estado: borrador provisional` (u otras variantes previas al sistema) se interpreta automáticamente como BORRADOR". Vale la pena que un encargo futuro añada esa regla de equivalencia a `CHAPTER_LIFECYCLE.md` antes de correr CLOSE sobre más capítulos heredados (C01-C29), para no repetir este análisis capítulo por capítulo.
